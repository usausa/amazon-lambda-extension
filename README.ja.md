...
| `ALE0018` | Warning | ハンドラ/引数 | `[FromCustomAuthorizer]` の誤用 |
...
- **設定ファイルは生成しない** — `serverless.template` / CloudFormation / SAM の生成・同期は行いません。`[FunctionUrl]` の CORS 設定や `Authorizer = nameof(...)` は属性値の保持・診断にのみ使用し、インフラ定義の出力には使いません。
